...
class TransferPessimisticLockTest: EmbeddedDbTest() {
    ...
    @Test
    fun `transfer from one account to another with transferPessimisticLock`() {
        val id1 = addAccount(10)
        val id2 = addAccount(5)

        db.testDatabase.connection.use { c ->
            val exchange = TransferPessimisticLock(c)
