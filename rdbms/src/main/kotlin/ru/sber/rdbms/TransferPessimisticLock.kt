...
class TransferPessimisticLock {
    fun transfer(accountId1: Long, accountId2: Long, amount: Long) {
        ...
        connection.use { conn ->
            ...
            try {
                ...
                val prepareStatement2 = conn.prepareStatement(SQL_UPDATE_AMOUNTS)
    ...
    companion object {
        val SQL_SELECT_AMOUNT = "select amount from bankaccount where id = ? for update;"
        val SQL_UPDATE_AMOUNTS =
            """update bankaccount set amount = amount - ?,  version = version + 1 where id = ?;
            update bankaccount set amount = amount + ?,  version = version + 1 where id = ?;"""
    }
}

fun main(){
    TransferPessimisticLock().transfer(1,2,1)
}